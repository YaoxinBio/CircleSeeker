...
In 1.6.0, N segments produce N directed junctions, including the last-to-first closure. Endpoints use one-base, zero-based half-open intervals at each segment's strand-aware entry and exit. LAST-derived MAPQ, low-MAPQ and confidence fields are missing because LAST does not provide MAPQ; identity is measured from alignment columns. Per-read RCA support remains distinct from molecule-level totals.
