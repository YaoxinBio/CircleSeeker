...
def select_best_duplicate(group: pd.DataFrame) -> pd.DataFrame:
    ...
    if prob_col:
        ...
        max_splits = group["num_split_reads"].max()
        if max_splits > 0:
            group["score"] = (group["num_split_reads"] / max_splits) * 0.6 + (
                1 - group.get("prob_artifact", 0)
            ) * 0.4
        else:
            ...
            with open(chimeric_fasta, "w") as f:
                ...
                for eccDNA_id, group in chimeric_df.groupby("eccDNA_id"):
                    ...
                    if segments:
                        ...
                        seg_desc = ";".join(seg_info)
                        header = (
                            f">{eccDNA_id}|segments={seg_count}|"
                            f"length={total_length}|type=chimeric|{seg_desc}"
                        )
                        f.write(header + "\n")
                        # Write sequence in 60-character lines
...
def write_curated_tables(
    simple_df: pd.DataFrame, chimeric_df: pd.DataFrame, output_prefix: Path | str
) -> tuple[Path, Path]:
    """
    ...
    results = write_curated_tables_with_fasta(
        simple_df, chimeric_df, output_prefix, organize_files=False
    )
