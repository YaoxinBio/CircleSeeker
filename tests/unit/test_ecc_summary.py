...
class TestFastaStatsCanComeFromTheFilterStep:
    """The 76 GB input is already walked by Sieve; summary should not walk it again.

    read_filter reads every read of the raw input to write the filtered FASTA.
    ecc_summary then read the same file a third time (after process_fasta and
    collect_read_statistics were merged into one) purely for total_sequences
    and total_length. Sieve can carry both numbers forward instead.
    """
    ...
    def test_supplied_counts_are_used_verbatim(self, tmp_path):
        fasta = self._fasta(tmp_path)
        summary = self._summary(tmp_path)
        scanned = summary._scan_fasta_by_line(fasta)
        ...
        monkey.setattr(builtins, "open", counting_open)
        try:
            stats = summary.process_fasta(fasta, counts=scanned)
        finally:
            monkey.undo()

        assert opened["count"] == 0, "walked the FASTA despite being handed the counts"
        assert stats["total_sequences"] == scanned[0]
        assert stats["total_length"] == scanned[1]
        assert stats["average_length"] == round(scanned[1] / scanned[0], 2)

    def test_supplied_counts_match_a_real_scan(self, tmp_path):
        fasta = self._fasta(tmp_path)
        summary = self._summary(tmp_path)

        supplied = summary.process_fasta(fasta, counts=summary._scan_fasta_by_line(fasta))
        fresh = self._summary(tmp_path).process_fasta(fasta)

        assert supplied["total_sequences"] == fresh["total_sequences"] == 3
        assert supplied["total_length"] == fresh["total_length"] == 13
        assert supplied["average_length"] == fresh["average_length"]

    def test_without_counts_it_still_scans(self, tmp_path):
        fasta = self._fasta(tmp_path)
        summary = self._summary(tmp_path)
        assert summary.process_fasta(fasta)["total_sequences"] == 3
